...
Transcribes and diarizes call audio using NeMo Parakeet + Pyannote.
CRITICAL FIX: Uses return_hypotheses=True to prevent empty transcripts.
"""
...
try:
    # Note: Newer pyannote uses 'token' instead of 'use_auth_token'
    diarization_pipeline = Pipeline.from_pretrained(
        'pyannote/speaker-diarization-3.1',
        use_auth_token=Config.HF_TOKEN  # Legacy parameter name for compatibility
    )
    ...
    try:
        diarization_pipeline = Pipeline.from_pretrained(
            'pyannote/speaker-diarization-3.1',
            token=Config.HF_TOKEN  # New parameter name (pyannote.audio >= 3.0)
        )
        diarization_pipeline.to(torch.device('cuda'))
...
def transcribe_audio(audio_path: str) -> str:
    ...
    Uses the simple list-based API: asr_model.transcribe([audio_path])
    This returns a list of transcription strings directly.
    """
    logger.info(f'🎤 Transcribing with Parakeet...')

    # Simple API: just pass a list of file paths
    results = asr_model.transcribe([audio_path])

    if not results or len(results) == 0:
        logger.warning('⚠️ Parakeet returned empty results')
        return ""
    ...
    if hasattr(hypothesis, 'text'):
        ...
        logger.warning(f'⚠️ Unexpected hypothesis type: {type(hypothesis)}')
        text = str(hypothesis)
...
def process_call() -> bool:
    ...
    try:
        ...
        # Transcribe (with return_hypotheses=True fix)
